...
async function fillTimePicker(){
    ...
    if(myJson.length != 0) {
        myJson.forEach((time) =>{
            output +=  `<option value="${time.gioDi}">${time.gioDi}</option>`
...
async function addCustomer(){
 ...
 fetch('http://localhost:5000/customer/insert',{
    ...
    .then(res => res.json())
    .then(res => res.status ? alert("Bạn đã đặt vé thành công") : alert("Có lỗi trong quá trình đặt vé! Thử lại"))
